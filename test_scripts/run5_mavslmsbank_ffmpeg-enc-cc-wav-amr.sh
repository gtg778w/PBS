...
#This script is a test script that will be used as a model for the scripts for all other 
#workloads.

#Note, the LOGDIR variable should be set with export or setenv by any calling shell
    if ( $#argv == 0 ) then
        set LOGDIR="log"
    else if ( $#argv == 1 ) then
        set LOGDIR=$argv[1]
    else
        echo "Usage:"$argv[0]" [log directory]"; exit 1
    endif
    ...
    mkdir -p ${LOGDIR}/${CONFIGNAME}
    set LOCALLOGDIR=${LOGDIR}"/"${CONFIGNAME}
    ...
    set alpha_array=("0.88803" "2.85383" "2.89286" "2.91969" "2.94895" "2.99042")
    ...
    foreach alpha ($alpha_array[1])
    
        #The suffix of the log file to store the data collected by the SRT application
        set SRT_logfilesuffix="_SRT_PeSoRTA_ffmpeg_"${CONFIGNAME}"_"${J1}"_"${A1}"_"${p1}"_"${c1}"_"${alpha}".log"        
        set ALC_logfilesuffix="_allocator_PeSoRTA_ffmpeg_"${Ta}"_"${Qa}"_"${sa}".log"
        ...
        foreach rep (`seq 1 1 ${repetitions}`)
            ...
            sleep 0.5
            #Run the SRT task
            bin/ffmpeg_pbsSRT -f -W ${W1} -D ${D1} -J ${J1} -A ${A1} -p ${p1} -c ${c1} -a ${alpha} -L 1 -R ${SRT_logfile} &
