#! /bin/csh -x
    ...
    sleep 1
    bin/sqrwavSRT -f -j 16 -P ${P1} -D ${D1} -d ${d1} -M ${M1} -m ${m1} -N ${N1} -p $p1} -b ${b1} -a ${a1} -L 1 -R /dev/null
    echo 0 > /proc/sched_pbs_actv
    echo "complete! "
    ...
    foreach i (`seq 1 1 ${repetitions}`)
        ...
        sleep 1
        bin/sqrwavSRT -f -j ${j1} -P ${P1} -D ${D1} -d ${d1} -M ${M1} -m ${m1} -N ${N1} -A ${A1} -p ${p1} -b ${b1} -a ${a1} -L 1 -R ${logfile1} &
        bin/sqrwavSRT -f -j ${j2} -P ${P2} -D ${D2} -d ${d2} -M ${M2} -m ${m2} -N ${N2} -A ${A2} -p ${p2} -b ${b2} -a ${a2} -L 1 -R ${logfile2} &
        bin/poll_pbs_actv
